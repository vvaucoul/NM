...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/11 13:25:26 by vvaucoul          #+#    #+#             */
/*   Updated: 2022/04/11 13:10:55 by vvaucoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/ft_nm.h"

static void init_section_info(t_symbol *sym, t_elf_64 *meta_data, size_t i)
{
    int shndx;
...
static t_symbol *get_symbols(t_elf_64 *meta_data)
{
    ...
    else
    {
        for (size_t i = 0; i < meta_data->sym_count; i++)
        {
            ...
            symbols[i].sym_type = ELF64_ST_TYPE(meta_data->sym[i].st_info);
            symbols[i].sym_bind = ELF64_ST_BIND(meta_data->sym[i].st_info);
            symbols[i].sym_vis = ELF64_ST_VISIBILITY(meta_data->sym[i].st_other);
...
int x64_execution(t_meta_file *meta_file)
{
    ...
    ehdr = (Elf64_Ehdr *)meta_file->ptr;
    if ((size_t)meta_file->st.st_size < (size_t)ehdr->e_shoff + (ehdr->e_shnum * ehdr->e_shentsize))
        return (display_error(E_FORMAT));
    if (setup_x64_meta_data(&meta_data, meta_file->ptr))
        return (FAILURE);
